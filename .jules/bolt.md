...
## 2024-03-24 - N+1 Fetch in Topic Progress
**Learning:** `update_progress_from_result` was fetching all topic progress rows for a user just to find one matching subject/topic. This is an O(N) memory and time bottleneck as the user completes more quizzes.
**Action:** Use targeted database queries for single-item lookups instead of fetching entire collections into application memory.
