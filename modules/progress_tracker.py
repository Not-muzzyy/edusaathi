...
from modules.auth import get_quiz_history, get_topic_progress, upsert_topic_progress, get_conn
import json
...
def update_progress_from_result(user_id, subject, topic, score, total):
    ...
    # 💡 What: Replaced O(N) application-side filtering with a single O(1) indexed database query.
    # 🎯 Why: Previously, the code fetched ALL topics for a user to find just one, causing memory and DB I/O bloat as history grows.
    # 📊 Impact: Eliminates unnecessary row fetching and application memory allocation, speeding up the submit endpoint.
    conn = get_conn()
    row = conn.execute(
    ...
    if row:
        mastery = round(0.7 * row["mastery_score"] + 0.3 * new_score, 3)
    else:
