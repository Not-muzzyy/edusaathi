...
def update_progress_from_result(user_id, subject, topic, score, total):
    ...
    # 💡 What: Replaced fetch of all user progress rows with targeted single-row lookup.
    # 🎯 Why: Previously, `get_topic_progress` fetched ALL topics and filtered in-memory, causing O(N) bottleneck.
    # 📊 Impact: DB query time goes from O(N) network transfer to O(1).
